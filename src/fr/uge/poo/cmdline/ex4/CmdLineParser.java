...
public class CmdLineParser {
	...
	private static class Option {
		...
		private Option(int nbParams, Required required, Status status, Consumer<List<String>> action) {
	...
	public void addOptionWithOneParameter(String option, Consumer<String> action) {
		...
		Consumer<List<String>> newConsumer = parameters -> {
			action.accept(parameters.get(0));
		};
	...
	public void registerWithParameter(String option, int nbParam, Required required, Consumer<List<String>> action) {
		Objects.requireNonNull(option);
		Objects.requireNonNull(action);
		options.put(option, new Option(nbParam, required, Status.UNSET, action));
		...
		for (var i = 0; i < nbArgs; i++) {
			if (arguments.hasNext()) {
				...
				if (parameter.charAt(0) == '-') {
					throw new IllegalArgumentException("Parameters can't strat with '-'");
				}
				res.add(parameter);
			} else {
